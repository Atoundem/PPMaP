...
public class Model1UI extends Dialog{

	protected Object result;
	public Shell shell;
	protected int model;
	protected String path, algoSelected;
	 ...
	 * @param style
	 */
	public Model1UI(Shell parent, int style) {
		super(parent, style);
	}

	public Model1UI(Shell parent, int model, String path, String algoSelected) {
		super(parent);
		this.model = model;
	...
	public void createContents(String title) {
		...
		restoreImgMenuItem.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				MaizeDevRate.restoreImage(path, lblFunctImageDR, model);
			}
		});
		copyMenuItem.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				BufferedImage image = MaizeDevUtils.convertToAWT(new ImageData(path));
		...
		btnAccept.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				finish = MessageDialog.openConfirm(shell, "Finish MaizeDevRate",
						"Are you sure to choose this function: "+shell.getText()+
						" for this Algorithm: "+ algoSelected +"?");
				if(finish)
					MaizeDevRate.setModelSelected(model);
			}
		});
