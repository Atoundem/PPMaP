...
//package org.cgiar.cip.ilcym.projects.clasesutils;
...
public class MaizeDevUtils {
	...
	public static void createTempScriptFile(String rScriptFile) {
	      
		File tempScripFile = new File(CreatedMaizeProject.getStrPathProject() + File.separator + "tempScripFile.r");
       
		InputStream in = MaizeDevUtils.class.getResourceAsStream(rScriptFile); 
        
        
        BufferedReader reader = new BufferedReader(new InputStreamReader(in));
        BufferedWriter writer;
		try {
			writer = new BufferedWriter(new FileWriter(tempScripFile));
        ...
        writer.close();
			...
			e.printStackTrace();
			MessageDialog.openError(new Shell(), "EPFA Warning", "Problems while trying to load Scripts");
		}
    }
...
*/
	...
	public static void getTemperaturesFromFile(String strFile){
		...
		try {
	        BufferedReader inLine = new BufferedReader(new FileReader(strFile));
	        BufferedWriter bw = new BufferedWriter(new FileWriter(MaizeDevUtils.ExtractPath(strFile) + "-1.txt"));
	        String str;
	        int line=1;
	        ...
	        c.eval("Table <- read.table("+'"'+ MaizeDevUtils.ExtractPath(strFile).replace("\\", "/") + "-1.txt" +'"'+",header=TRUE)");
	        c.close();
	    } catch (Exception e1) {
	    	e1.printStackTrace();
	    	c.close();
