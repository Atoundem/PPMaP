...
*/
	...
	public static void calculateIndices(String strR, String pathTempMin,String pathTempMax,
    		String dbMinX, String dbMaxX, String dbMinY, String dbMaxY, String strOutput, boolean rbFilter, String tmin, String tmax, String season)
	{
...
//		}
    	...
    	//String lib = MaizeDevUtils.getScriptPath();
    	try{
    		
    		MaizeDevUtils.createTempScriptFile("/Scripts/spatialTransform.r");
			...
			System.out.println("Scrip load!");  //  "/" + "MaizeDevRate-"+MainPageWizardPage.comboDevStage.getText() + ".RData"
			//file = "+ '"' + MainPageWizardPage.getstrMortalityPath() + "/" + "MaizeDevRate-"+MainPageWizardPage.comboDevStage.getText() + ".RData"
			
			savetoR +="load("+ '"' + MainPageWizardPage.getstrMortalityPathMapping(MappingDialog.comboDevStage.getText()) + "/" + "MaizeDevRate-"+MappingDialog.comboDevStage.getText() + ".RData"+'"'+")"+"\n";
			c.eval("load("+ '"' +MainPageWizardPage.getstrMortalityPathMapping(MappingDialog.comboDevStage.getText()) + "/" + "MaizeDevRate-"+MappingDialog.comboDevStage.getText() + ".RData"+'"'+")");
...
//			c.eval("steps <- 48");
			...
			if(MappingDialog.btnPeriodicity.getSelection())
			{
				savetoR += "season=c("+season + ")" + "\r\n";
				c.eval("season=c("+season + ")");
...
//  		file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,sol_develop$ecuacion,filtro,tfunc,estshap,alg)			
			...
			savetoR += "file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,sol_develop$ecuacion,filtro=filtro,tfunc,estshap,alg,season)"+"\n";
			c.eval("file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,sol_develop$ecuacion,filtro=filtro,tfunc,estshap,alg,season)");
			
/*			if(!MappingDialog.btnLimitDifference.getSelection()){
				savetoR += "file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,filtro=filtro,coefEstimated,gg,alg)"+"\n";
				c.eval("file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,filtro=filtro,coefEstimated,gg,alg)");
			}else{
				savetoR += "file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,filtro=filtro,coefEstimated,gg,alg)"+"\n";
				c.eval("file<-zone.div(dir1,dir2,ilon,ilat,R,dir.out,name.out=name.out,filtro=filtro,coefEstimated,gg,alg)");
			}
