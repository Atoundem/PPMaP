...
dat[,Area] = as.character(dat[,Area])
  ...
  for (z in unique(dat[,Area]))
  {
    ...
    NberStep = nrow(devStage)
    allDevPeriod = c(which(dat[,dates]== devStage[NberStep,3]& dat[,'Locations']== z ), which(dat[,dates]== devStage[1,2]& dat[,'Locations']== z )) #A vector of indices of the beginning and ending date of developement in the data file.
    for (i in 1:NberStep)
    {
      stepPeriod = c(which(dat[,dates]== devStage[i,2]& dat[,'Locations']== z ), which(dat[,dates]== devStage[i,3]& dat[,'Locations']== z ))  # we verify that starting and the ending data of the step belong to the file of data. in return their index
    ...
    MinTemp = mean(dat[allDevPeriod[2]:allDevPeriod[1],2]); MaxTemp = mean(dat[allDevPeriod[2]:allDevPeriod[1],3]) ; Temp = round(mean(MinTemp,MaxTemp),3)
