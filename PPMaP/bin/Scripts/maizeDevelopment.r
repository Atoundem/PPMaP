...
pruebamortal<-function(modelNumber,datm,inim,corrx,corry,mini,maxi,labx, laby,titulo)
{
	...
	plot(datm[,1],datm[,2],frame=F,pch=19,col=4,cex=1.3,xlim=corrx,ylim=corry,axes=F,xaxt = "n",xlab=labx,ylab=laby,main=titulo)
	
	axis(1, xaxp=c(corrx,5))
	#axis(2,las=2)
	axis(side=2,seq(corry[1],corry[2],(corry[2]-corry[1])/(length(datm[,2])-1)),las=2)
	ini<-as.list(inim)
	for (i in names(ini))
	{
		temp <- ini[[i]]
	...
	ylinea<-eval(form)
	lines(xlinea,ylinea,lwd=2,col=2)
...
#  }
  ...
  if(limit=="yes")
  {
	  ...
	  #if(is.null(corry)){corry=c(0,max(data[,2])*1.01);corry2=seq(0,round(max(corry)),10)}else{corry2=seq(corry[1],corry[2],(corry[2]-corry[1])/(length(datm[,2])))}              ## cambio  // can use this when dividing Y with number of parameter
	  
	  if(is.null(corry)){corry=c(0,max(data[,2])*1.15);corry2=seq(0,max(corry),(corry[2]-corry[1])/nbY)}else{corry2=seq(corry[1],corry[2],(corry[2]-corry[1])/nbY)}              ## cambio
      
      plot(data[,1],data[,2],frame=F,col=ccol[1],xlim=corrx,ylim=c(corry[1],k*corry[2]),,xlab=labx,ylab=laby,pch=19,axes=F,xaxt = "n",main=titulo) ## 4
      
	  #axis(side=2,seq(corry[1],corry[2],(corry[2]-corry[1])/(length(datm[,2]-1))),las=2)
      axis(1, corrx2)  ## cambio
	 # axis(side=2,round(seq(corry[1],k*corry[2],(k*corry[2]-corry[1])/(length(datm[,2]))),5),las=2)  // this divide Y according number of parameter
...
#      axis(2, corry2,las=2) ## cambio
  ...
  if(limit=="no")
  {
      ...
      par(cex=tam)
      if(is.null(corrx)){corrx=c(min(data[,1],0),max(data[,1])*1.2);corrx2=seq(0,10*round(corrx[2]/10,1),5)}else{corrx2=seq(corrx[1],corrx[2],5)} ## cambio
      if(is.null(corry)){corry=c(0,(max(data[,2])*1.1));corry2=seq(0,max(corry),(corry[2]-corry[1])/nbY)}else{corry2=seq(corry[1],corry[2],(corry[2]-corry[1])/(length(datm[,2])))}              ## cambio
