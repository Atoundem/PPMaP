library(minpack.lm) #install.packages("minpack.lm", dependencies = T) #for the function nls.lm()
library(MASS) #install.packages("MASS", dependencies = T) # for the function ginv()

TransformDAta <- function(dataFile,devStageFile)
{
  
  #,xRange,yRange,xLabel, yLabel,graphTitle )
  #####################################################################################
  #This function take as input a file collected from field on the development of a plant
  #and return the tranformation made on these data to make them usable for the modelisation
  #
  #dataFile : file of information colected form the field
  #
  #stageEndOfDate : the different stage of developement and their coresponding beginning and ending date.
  #
  #####################################################################################
  ...
  Globalresul =NULL
  GlobalTempInfos =NULL
  FinalDatm = NULL
  FinDat =NULL
  
  #dirFile = paste("./Data/",dataFile,sep="") 
  ...
  #dirFile = paste("./Data/",devStageFile,sep="")
  fileDevStage = read.table(devStageFile, head = T)
  #for(i in 1:(length(fileDevStage)-2))
  for(i in 1:Area)
    fileDevStage[,i] = as.character(fileDevStage[,i])
  
  #NberStep = nrow(devStage)
  
  for (z in unique(dat[,Area]))
  {
    AlternatifResult = NULL
    devStage = fileDevStage[which( fileDevStage['Location']== z),]
    NberStep = nrow(devStage)
    allDevPeriod = c(which(dat[,dates]== devStage[NberStep,3]& dat[,Area]== z ), which(dat[,dates]== devStage[1,2]& dat[,Area]== z )) #A vector of indices of the beginning and ending date of developement in the data file.
    for (i in 1:NberStep)
    {
      stepPeriod = c(which(dat[,dates]== devStage[i,2]& dat[,Area]== z ), which(dat[,dates]== devStage[i,3]& dat[,Area]== z ))  # we verify that starting and the ending data of the step belong to the file of data. in return their index
      
      
      duration = stepPeriod[2]-stepPeriod[1]+1 # the duration id the difference between ending and starting date of the step
      
      StepMeanTemp = c(mean(dat[stepPeriod[1]:stepPeriod[2],2]),mean(dat[stepPeriod[1]:stepPeriod[2],3])) # we compute the minimun and maximun mean temperature for the giving step
      result = data.frame(StepName =devStage[i,1] , StepDevelpTime = duration, StepDevelpRate =round(1/duration,5), StepMinTemp =round(StepMeanTemp[1],3) , StepMaxTemp =round(StepMeanTemp[2],3), AllDevTime = round(allDevPeriod[1]-allDevPeriod[2]+1,3),AllDevRate = round(1/(allDevPeriod[1]-allDevPeriod[2]+1),5) , Location = z)#, Lat= devStage[1,length(devStage)-1], Long = devStage[1,length(devStage)])
      
      AlternatifResult = rbind(AlternatifResult,result)
      
    }
    
    Globalresul = rbind(Globalresul,AlternatifResult)
    #allDevPeriod =  c(match(devStage[i,2:3],dat[,dates]),match(devStage[i,2:3],dat[,dates]))
    
    MinTemp = mean(dat[allDevPeriod[2]:allDevPeriod[1],2]); MaxTemp = mean(dat[allDevPeriod[2]:allDevPeriod[1],3]) ; Temp = round(mean(c(MinTemp,MaxTemp)),3)
    #GlobalTempInfos = data.frame(Location = z,GlobalMinTemp = MinTemp,GlobalMaxTemp = MaxTemp,GlobalTemp = Temp)
    GlobalTempInfos = rbind(GlobalTempInfos,data.frame(Location = z,GlobalMinTemp = MinTemp,GlobalMaxTemp = MaxTemp,T = Temp))
    
    FinalDatm = rbind(FinalDatm,data.frame(T =Temp , DevRate = 1/(allDevPeriod[1]-allDevPeriod[2]+1),Area =z))
    #FinDat = rbind(FinDat,data.frame(T =Temp , DevRate = 1/(allDevPeriod[1]-allDevPeriod[2]+1),Location =z))
  }
  val = NULL
  #construction of the different result 
  for (stp in unique(Globalresul$StepName))
    
    val = rbind(val,data.frame( T =GlobalTempInfos['T'],Globalresul[Globalresul['StepName']== stp,c("StepDevelpRate",'Location','StepName')]))#,'Lat','Long'
  
  result = split(val,val['StepName'])
  tabl = result
  
  
  #we divide the plotting windows in the number of step for the process
...
######### Set of new funcion added
		...
		if(model==3 || model==4 || model==5 || model==6 || model==10 || model==11 || model==13 || model==14|| model==61) {lines(lineashap-273.15,ylineash,col=ccol[3],lwd=2)}else{lines(lineashap,ylineash,col=ccol[3],lwd=2)} ##  3
...
#	if(is.null(corry)){corry=c(0,max(data[,2])*1.15);corry2=seq(0,max(corry),(corry[2]-corry[1])/nbY)}else{corry2=seq(corry[1],corry[2],scaleY)}              ## cambio
	...
	if(is.null(corry)){corry=c(0,max(data[,2])*1.5);corry2=seq(0,max(corry),0.1)}else{corry2=seq(corry[1],corry[2],scaleY)}              ## cambio
...
#	axis(side=2,round(seq(corry[1],k*corry[2],(k*corry[2]-corry[1])/nbY),5),las=2)
	...
	if(limit=="yes"){
		...
		if(model==3 || model==4 || model==5 || model==6 || model==7 || model==8 || model==9 || model==10 || model==11 || model==12 || model==13 || model==14|| model==61){  ############ solo aumente el tipo de modelo
	...
	if(limit=="no") if(model==3 || model==4 || model==5  || model==6 || model==10 || model==11 || model==12 || model==13 || model==14 || model==61) lines(xl-273.15, yl, lwd=2,col=ccol[2]) else lines(xl, yl, lwd=2,col=ccol[2]) ## 2 ..... solo aumente el tipo de modelo
...
############################
		...
		## funciones adaptadas a la senescencia
				...
				if(model==60)   {
					...
					yl<-eval(expre)
					gg<-"r(T) = Rmax*((Tceil-x)/(Tceil-Topc))*((x/Topc)^(Topc/(Tceil-Topc)))"
                ...
                if(model==61)   {
					...
					expre<-expression((51559240052*x*(exp(-73900/(8.314*x))))/(1+(exp(-73900/(8.314*x)))^(alpha*(1-x)/To)))
					yl<-eval(expre)
					gg<-"r(T) = (51559240052*x*(exp(-73900/(8.314*x))))/(1+(exp(-73900/(8.314*x)))^(alpha*(1-x)/To))"
                    ggg<-y ~ (51559240052*x*(exp(-73900/(8.314*x))))/(1+(exp(-73900/(8.314*x)))^(alpha*(1-x)/To))          
			...
			if(model==62) {
					...
					yl<-eval(expre)
					gg<-"r(T) = exp(k).((x-Tb)^alpha).((Tc-x)^betas)"
				}
			...
			if(model==63){
				...
				gg<-"r(T) = (2.((x-Tmin)^(log(2)/log((Tmax-Tmin)/(Topt-Tmin)))).((Topt-Tmin)^(log(2)/log((Tmax-Tmin)/(Topt-Tmin))))-((x-Tmin)^(2.(log(2)/log((Tmax-Tmin)/(Topt-Tmin))))))/((Topt-Tmin)^(2.(log(2)/log((Tmax-Tmin)/(Topt-Tmin)))))"
			...
			if(model==64) {
				...
				gg<-"r(T) = Yasym/(1+v.exp(-k.(x-Tm)))^(1/v)"
			...
			if(model==66) {
				...
				gg<-"r(T) = Rmax.(1+((Tmax-x)/(Tmax-Topt))).(x/Tmax)^(Tmax/(Tmax-Topt))"
			...
			if(model==69){
				...
				gg<-"r(T) = Rmax.((Tmax-x)/(Tmax-Topt)).(x/Tmax)^(Tmax/(Tmax-Topt))"
			...
			if(model==70) {
				...
				gg<-"r(T) = Yasym.exp(a.(x-Topt)^2 + b.(x-Topt)^3)"
			...
			if(model==71){
				...
				gg<-"r(T) = Yasym.exp(-0.5((x-Topt)/b)^2)"
			...
			if(model==72) {
				...
				gg<-"r(T) = Rmax.((Tmax-x)/(Tmax-Topt)).((x-Tmin)/(Topt-Tmin))^((Topt-Tmin)/(Tmax-Topt))"
			...
			if(model==74) {
				...
				gg<-"r(T) = Yb+(Rmax-Yb).(1+((Tmax-x)/(Tmax-Topt))).(((x-Tmin)/(Tmax-Tmin))^((Tmax-Tmin)/(Tmax-Topt)))"
			...
			if(model==76) {
				...
				gg<-"r(T) = Rmax.(3Tmax-2xl)*(x)^2/(Tmax)^3"
			...
			if(model==77){
				...
				gg<-"r(T) = Rmax.(1-(1+((Tmax-x)/(Tmax-Topt))).((x/Tmax)^(Tmax/(Tmax-Topt))))"
			...
			if(model==81) {
				...
				gg<-"r(T) = a.exp(-0.5((log(x/Topt)/b)^2))"
			...
			if(model==82) { 
				...
				gg<-"r(T) = a.((k/(1+(((x-Topt)/b)^2)))+(1-k).exp(-0.5(((x-Topt)/b)^2)))"
...
#                AC<-length(x)*(log(2*pi*sqe/length(x))+1)+2*(length(estshap)+1) #Cambio segun Nonlinear Regression with R, pag 105. DM
                ...
                if (model==1 || model==2 || model==3 || model==4 || model==5 || model==6 || model==7 || model==8 || model==9 || model==10 || model==11 || model==12 || model==13 || model==14|| model==61){
